...
Based on the business intent, propose one application and return only:
...
This is not enterprise architecture, a landing zone, an organization-wide control plane, or a list of architecture alternatives. Do not use layers named "Experience", "Governance", "Catalog", "Enterprise systems", or "Platform landing zone" unless the product itself is a low-code runtime and the selected style is Low-code application runtime.

Rules:
- Name concrete application components (e.g. ClaimsIntake API, AdjusterWorkbench UI, ReservationService).
- Map layers to: presentation, api, domain, data, integration, and optionally cross-cutting.
- The Mermaid diagram must be a layered application diagram using subgraphs named Presentation, API, Domain, Data, Integrations. Start with graph TB. Quoted labels only.
